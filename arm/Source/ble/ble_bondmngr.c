...
/**
 * @defgroup ble_bond_mngr_init_macros  Macros to verify Module Initialization Status
 * @brief    This group has a macro to verify this module's initialization status.
 * @{
 */
...
static void update_whitelist(void)
{
    ...
    for (i = 0, m_addr_count = 0, m_irk_count = 0; i < m_masters_in_db_count; i++)
    {
        ...
        if ((p_bond->auth_status.central_kex.irk) &&
           (BLE_GAP_ADDR_TYPE_RANDOM_PRIVATE_RESOLVABLE == p_bond->master_addr.addr_type))
        {
...
static void on_sys_attr_missing(ble_evt_t * p_ble_evt)
{
    ...
    if ((m_master.bond.master_handle == INVALID_MASTER_HANDLE) || (!GET_CONN_SEC_UPDATE_RCVD()))
    {
        err_code = sd_ble_gatts_sys_attr_set(m_conn_handle, NULL, 0);
    }
    else
...
static void on_auth_status(ble_gap_evt_auth_status_t * p_auth_status)
{
    ...
    if (false == GET_CONN_SEC_UPDATE_RCVD())
    {
        return;
    }
